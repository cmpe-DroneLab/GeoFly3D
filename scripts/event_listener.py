...
from olympe.messages.common.CalibrationState import MagnetoCalibrationRequiredState, MagnetoCalibrationAxisToCalibrateChanged, MagnetoCalibrationStartedChanged, MagnetoCalibrationStateChanged
...
def print_event(event):
    ...
    if isinstance(event, olympe.ArsdkMessageEvent):
        max_args_size = 60
        args = str(event.args)
        args = (args[: max_args_size - 3] +
                "...") if len(args) > max_args_size else args
...
class EventListener(olympe.EventListener):
    ...
    # https://developer.parrot.com/docs/olympe/arsdkng_battery.html#olympe.messages.battery.alert
    @olympe.listen_event(battery_alert(_policy="wait"))
    def onBatteryAlert(self, event, scheduler):
        ...
        if alert['level'] is alert_level.warning:
            rospy.logwarn_throttle(
                60, "Battery Allert: " + alert['alert'].name)
    ...
    def onAlertStateChanged(self, event, scheduler):
        ...
        # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.enums.ardrone3.PilotingState.AlertStateChanged_State
        if alert_state is not AlertStateChanged_State.none and alert_state is not AlertStateChanged_State.user:
            rospy.logwarn_throttle(60, "Alert: " + alert_state.name)

    # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.messages.ardrone3.PilotingState.ForcedLandingAutoTrigger
    @olympe.listen_event(ForcedLandingAutoTrigger(_policy="wait"))
    def onForcedLandingAutoTrigger(self, event, scheduler):
        ...
        if forced_landing is not ForcedLandingAutoTrigger_Reason.NONE:
            # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.enums.ardrone3.PilotingState.ForcedLandingAutoTrigger_Reason
            rospy.logwarn_throttle(
                60, "Forced Landing Reason: " + forced_landing.name)

    # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.messages.ardrone3.PilotingState.HoveringWarning
    @olympe.listen_event(HoveringWarning(_policy="wait"))
    def onHoveringWarning(self, event, scheduler):
        ...
        if (self.drone.state == "HOVERING" or self.drone.state == "FLYING"):
            if hovering_warning['no_gps_too_dark']:
                rospy.logwarn_throttle(
                    60, "Hovering Warning: the drone doesn’t have a GPS fix AND there is not enough light")
            if hovering_warning['no_gps_too_high']:
                rospy.logwarn_throttle(
                    60, "Hovering Warning: the drone doesn’t have a GPS fix AND is flying too high")

    # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.messages.ardrone3.PilotingState.VibrationLevelChanged
    @olympe.listen_event(VibrationLevelChanged(_policy="wait"))
    def onVibrationLevelChanged(self, event, scheduler):
        vibration_level = event.args['state']
        # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.enums.ardrone3.PilotingState.VibrationLevelChanged_State
        if vibration_level is not VibrationLevelChanged_State.ok:
            rospy.logwarn_throttle(
                60, "Vibration Level: " + vibration_level.name)

    # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.messages.ardrone3.PilotingState.WindStateChanged
    @olympe.listen_event(WindStateChanged(_policy="wait"))
    def onWindStateChanged(self, event, scheduler):
        wind_state = event.args['state']
        # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.enums.ardrone3.PilotingState.WindStateChanged_State
        if wind_state is not WindStateChanged_State.ok:
            rospy.logwarn_throttle(60, "Wind State: " + wind_state.name)

    # https://developer.parrot.com/docs/olympe/arsdkng_common_common.html#olympe.messages.common.CommonState.LinkSignalQuality
    @olympe.listen_event(LinkSignalQuality(_policy="wait"))
    def onAlertStateChanged(self, event, scheduler):
        link_quality = event.args['value']
        if (link_quality >> 6) & 1:
            rospy.logwarn_throttle(
                60, "4G interference coming from the smartphone")
        if (link_quality >> 7) & 1:
            rospy.logwarn_throttle(
                60, "Radio link is perturbed by external elements")

    # https://developer.parrot.com/docs/olympe/arsdkng_common_common.html#olympe.messages.common.CommonState.MassStorageInfoStateListChanged
    @olympe.listen_event(MassStorageInfoStateListChanged(_policy="wait"))
    ...
    # https://developer.parrot.com/docs/olympe/arsdkng_common_common.html#olympe.messages.common.CommonState.SensorsStatesListChanged
    @olympe.listen_event(SensorsStatesListChanged())
    ...
    # https://developer.parrot.com/docs/olympe/arsdkng_rth.html#olympe.messages.rth.home_reachability
    @olympe.listen_event(home_reachability_message(_policy="wait"))
    def on_home_reachability(self, event, scheduler):
        # https://developer.parrot.com/docs/olympe/arsdkng_rth.html#olympe.enums.rth.home_reachability
        home_reachability = event.args['status']
    ...
    # https://developer.parrot.com/docs/olympe/arsdkng_rth.html#olympe.messages.rth.rth_auto_trigger
    @olympe.listen_event(rth_auto_trigger(_policy="wait"))
    def on_home_reachability(self, event, scheduler):
        # https://developer.parrot.com/docs/olympe/arsdkng_rth.html#olympe.enums.rth.auto_trigger_reason
        rth_auto_trigger = event.args['reason']
    ...
    # https://developer.parrot.com/docs/olympe/arsdkng_drone_manager.html#olympe.messages.drone_manager.connection_state
    @olympe.listen_event(connection_state(_policy="wait"))
    def on_connection_state(self, event, scheduler):
        # print(str(event.args))
        rospy.loginfo(get_timestamp() +
                      " connection_state: " + str(event.args))
        # self.connection_callback(event)

    # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.messages.ardrone3.PilotingState.FlyingStateChanged
    @olympe.listen_event(FlyingStateChanged(_policy="wait"))
    def on_connection_state(self, event, scheduler):
        # print(str(event.args))
        rospy.loginfo(get_timestamp() + " flying_state: " +
                      str(event.args['state']))
        # self.connection_callback(event)
    ...
    @olympe.listen_event(MagnetoCalibrationRequiredState(_policy="wait"))
    def on_calibration_required(self, event, scheduler):
        rospy.loginfo(get_timestamp() + ' Calibration Required: ' +
                      str(event.args['required']))

    # https://developer.parrot.com/docs/olympe/arsdkng_common_calibration.html#olympe.messages.common.CalibrationState.MagnetoCalibrationStartedChanged
    @olympe.listen_event(MagnetoCalibrationStartedChanged(_policy="wait"))
    def on_calibration_started(self, event, scheduler):
        rospy.loginfo(get_timestamp() + ' Calibration Started: ' +
                      str(event.args['started']))
    ...
    @olympe.listen_event(MagnetoCalibrationStateChanged(_policy="wait"))
    def on_calibration_changed(self, event, scheduler):
        rospy.loginfo(get_timestamp() +
                      ' Calibration State: ' + str(event.args))

    # https://developer.parrot.com/docs/olympe/arsdkng_common_calibration.html#olympe.messages.common.CalibrationState.MagnetoCalibrationAxisToCalibrateChanged
    @olympe.listen_event(MagnetoCalibrationAxisToCalibrateChanged(_policy="wait"))
    def on_axis_changed(self, event, scheduler):
        rospy.loginfo(get_timestamp() + ' Axis to Calibrate:  ' +
                      str(event.args['axis']))

    # https://developer.parrot.com/docs/olympe/arsdkng_move.html#olympe.messages.move.info
    @olympe.listen_event(move_info(_policy="wait"))
    def on_move_info(self, event, scheduler):
        move_info = event.args
        rospy.loginfo(get_timestamp() + ' Move info: ' + str(move_info))

    @olympe.listen_event(moveToChanged(_policy="wait"))
    ...
    @olympe.listen_event(user_storage_info(_policy="wait"))
    def on_user_storage_info(self, event, scheduler):
        capacity = event.args['capacity']
        # https://developer.parrot.com/docs/olympe/arsdkng_user_storage.html#olympe.messages.user_storage.monitor
        available_bytes = self.drone.get_state(user_storage_monitor)[
            'available_bytes']
        rospy.loginfo_once('%s Available space: %.1f/%.1fGB',
                           get_timestamp(), available_bytes/(2**30), capacity/(2**30))
    ...
    # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_gps.html#olympe.messages.ardrone3.GPSState.NumberOfSatelliteChanged
    @olympe.listen_event(NumberOfSatelliteChanged(_policy="wait"))
    def onNumberOfSatelliteChanged(self, event, scheduler):
        rospy.loginfo(get_timestamp() +
                      " Number of Satellite: " + str(event.args))
    ...
    # https://developer.parrot.com/docs/olympe/arsdkng_common_common.html#olympe.messages.common.CommonState.AllStatesChanged
    @olympe.listen_event(AllStatesChanged(_policy="wait"))
    def onAllStatesChange(self, event, scheduler):
        rospy.logdebug(get_timestamp() +
                       " AllStatesChanged: " + str(event.args))

    # https://developer.parrot.com/docs/olympe/arsdkng_common_common.html#olympe.messages.common.CommonState.BatteryStateChanged
    @olympe.listen_event(BatteryStateChanged(_policy="wait"))
    def onBatteryStateChange(self, event, scheduler):
        rospy.loginfo(get_timestamp() + " Battery State: " +
                      str(event.args['percent']))

    # https://developer.parrot.com/docs/olympe/arsdkng_battery.html#olympe.messages.battery.health
    @olympe.listen_event(health(_policy="wait"))
    def onBatteryHealth(self, event, scheduler):
        rospy.loginfo(get_timestamp() + " Battery Health: " + str(event.args))

    # https://developer.parrot.com/docs/olympe/arsdkng_battery.html#olympe.messages.battery.capacity
    @olympe.listen_event(capacity(_policy="wait"))
    def onBatteryCapacity(self, event, scheduler):
        rospy.loginfo(get_timestamp() +
                      " Battery Capacity: " + str(event.args))
    ...
    def on_user_storage_monitor(self, event, scheduler):
        rospy.loginfo(get_timestamp() + " Available bytes: " + str(event.args['available_bytes']))
        print()

    @olympe.listen_event(photo_progress(_policy="wait"))  # https://developer.parrot.com/docs/olympe/arsdkng_camera.html#olympe.messages.camera.photo_progress
    def on_user_storage_monitor(self, event, scheduler):
        print()
        rospy.loginfo(get_timestamp() + " Photo progress: " + str(event.args))
        print()
