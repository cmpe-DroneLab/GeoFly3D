...
class EventListener(olympe.EventListener):
	...
	@olympe.listen_event(FlyingStateChanged(_policy="wait"))  # https://developer.parrot.com/docs/olympe/arsdkng_ardrone3_piloting.html#olympe.messages.ardrone3.PilotingState.FlyingStateChanged
	def on_connection_state(self, event, scheduler):
		# print(str(event.args))
		rospy.loginfo(get_timestamp() + " flying_state: " + str(event.args['state']))
		# self.connection_callback(event)
	...
	@olympe.listen_event(MagnetoCalibrationRequiredState(_policy="wait"))  # https://developer.parrot.com/docs/olympe/arsdkng_common_calibration.html#olympe.messages.common.CalibrationState.MagnetoCalibrationRequiredState
	def on_calibration_required(self, event, scheduler):
		rospy.loginfo(get_timestamp() + ' Calibration Required: ' + str(event.args['required']))
	
	@olympe.listen_event(MagnetoCalibrationStartedChanged(_policy="wait"))  # https://developer.parrot.com/docs/olympe/arsdkng_common_calibration.html#olympe.messages.common.CalibrationState.MagnetoCalibrationStartedChanged
	def on_calibration_started(self, event, scheduler):
		rospy.loginfo(get_timestamp() + ' Calibration Started: ' + str(event.args['started']))

	@olympe.listen_event(MagnetoCalibrationStateChanged(_policy="wait"))  # https://developer.parrot.com/docs/olympe/arsdkng_common_calibration.html#olympe.messages.common.CalibrationState.MagnetoCalibrationStateChanged
	def on_calibration_changed(self, event, scheduler):
		rospy.loginfo(get_timestamp() + ' Calibration State: ' + str(event.args))

	@olympe.listen_event(MagnetoCalibrationAxisToCalibrateChanged(_policy="wait"))  # https://developer.parrot.com/docs/olympe/arsdkng_common_calibration.html#olympe.messages.common.CalibrationState.MagnetoCalibrationAxisToCalibrateChanged
	def on_axis_changed(self, event, scheduler):
		rospy.loginfo(get_timestamp() + ' Axis to Calibrate:  ' + str(event.args['axis']))
	...
	def onBatteryStateChange(self, event, scheduler):
		rospy.loginfo(get_timestamp() + " Battery State: " + str(event.args['percent']))
