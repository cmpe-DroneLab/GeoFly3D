...
def create_sparse_route(start_node,height,vertex_list):
    route           = []
    ...
    direction_horizontal, direction_vertical, distance_horizontal, distance_vertical  = calculate_boundaries(start_vertex,vertex_list)
    ...
    while counter_horizontal <= distance_horizontal:

        route.append(Node(current_latitude, current_longitude, 0))
        current_latitude, current_longitude = hs.inverse_haversine((current_latitude, current_longitude), distance_vertical, direction_vertical, unit=Unit.METERS)
        route.append(Node(current_latitude, current_longitude, 0))
...
route, _ = create_sparse_route(takeoff, 100, corner_list)
